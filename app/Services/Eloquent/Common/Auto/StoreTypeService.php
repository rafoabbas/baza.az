...
class StoreTypeService extends CrudService
{
    use ValidationRole;

    public function __construct(StoreTypeRepositoryInterface $repository)
    {
    ...
    public static function postRules(): array
    {
        return RuleHelper::make([
            '%name%' => 'required|string|max:64|unique:store_types,%name%',
    ...
    public static function putRules(): array
    {
        return array_merge(self::postRules(), RuleHelper::make([
            '%name%' => 'required|string|max:64|unique:store_types,%name%,'
                . request()->route('store_type')->getAttribute('id'),
        ]));
    }
}
