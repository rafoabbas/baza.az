...
class SpecificationService extends CrudService
{
    use ValidationRole;

    public function __construct(SpecificationRepositoryInterface $repository)
    {
    ...
    public static function postRules(): array
    {
        return RuleHelper::make([
            '%name%' => 'required|string|max:64|unique:specifications,%name%',
    ...
    public static function putRules(): array
    {
        return array_merge(self::postRules(), RuleHelper::make([
            '%name%' => 'required|string|max:64|unique:specifications,%name%,'
                . request()->route('specification')->getAttribute('id'),
        ]));
    }
}
